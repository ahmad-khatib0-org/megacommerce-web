  ...
  return (
    <Popover
      ...
      <Popover.Dropdown className='max-h-80 overflow-y-auto z-10'>
        {uploader.uploads.map((upload, idx) => <div key={idx} className="flex flex-col gap-y-2 py-4">
          ...
          {(upload.uploading ?? false) && <div className="flex items-center gap-x-2">
            <div className="relative size-7">
              <Image src={AssetsPaths.uploadFileGif} alt={tr.uploading} sizes="100%" fill unoptimized />
            </div>
            <p>{tr.uploading}</p>
          </div>}
          {(upload.completed ?? false) && <div className="flex items-center gap-x-2">
            <IconSquareCheckFilled color="green" />
            <p>{tr.completed}</p>
          </div>}
          {(upload.errorMsg?.length ?? 0) > 0 && <div className="flex items-center gap-x-2">
