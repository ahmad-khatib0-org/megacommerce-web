...
export type ProductCreateOfferWithVariationsForm =
  UseFormReturnType<ProductCreateOfferWithVariationsFormValues>
...
const ProductCreateOfferWithVariations = forwardRef<ProductCreateOfferWithVariationsHandler, Props>(
  ({ tr, offering, lang }, ref) => {
    ...
    const onToggleMinOrder = (e: ChangeEvent<HTMLInputElement>, idx: number) => {
      ...
      if (value) {
        form.setFieldValue(`variations.${idx}.minimum_orders`, [
          { id: Date.now().toString(), price: 0, quantity: 0 },
        ])
        ...
        {Object.keys(form.values.variations).map((field, idx) => {
          return (
            <div
              ...
              <div className='border border-orange-700 py-3 px-2 rounded-md'>
                <p className='font-semibold'>
                  {translator(lang, 'products.offer.variant_pricing', {
                    Title: form.values.variations[idx].title,
                  })}
                </p>
              </div>
              <TextInput
              ...
              />
              {(form.values.variations[idx].minimum_orders?.length ?? 0) > 0 && (
