...
const ProductCreateDetailsWithVariations = forwardRef<ProductCreateDetailsHandlers, Props>(
        ...
        {Object.keys(fieldsShared).length > 0 && (
          <>
            ...
            {Object.entries(fieldsShared).map(([fieldName, fieldData]) => {
              return (
                <ProductCreateDetailsInputWrapper
                  key={fieldName}
        ...
        {Object.keys(fieldsVariations).length > 0 && (
          <>
            ...
            {variationForm.values.variations.map((variation, idx) => {
              return (
                <div key={idx} className='flex flex-col border border-black/25 rounded-md px-2 py-4 gap-y-4'>
                  <TextInput
                    label={
                  ...
                  {idx !== 0 && (
                    <Button onClick={() => removeVariant(idx)} className='w-max px-4' variant='outline'>
                      {tr.rmVar}
                    </Button>
                  )}
                </div>
            ...
            <div className='flex justify-end'>
              <Button onClick={() => addAnotherVar()} variant='outline'>
                {tr.addVar}
              </Button>
            </div>
          </>
