...
function ProductCreateOfferWithoutVariations({ tr, offering, form }: Props) {
  ...
  return (
    <>
      ...
      />
      {(form.values.minimum_orders?.length ?? 0) > 0 && (
        <div className='flex flex-col gap-y-4'>
          {(form.values.minimum_orders ?? []).map((mo, idx) => (
            <div key={mo.id} className={`relative grid grid-cols-2 justify-center items-center gap-x-4 pt-2`}>
              {idx > 0 && (
                <div
